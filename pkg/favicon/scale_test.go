...
func TestScale(t *testing.T) {
	if err := os.MkdirAll("cache/hostname", 0744); err != nil {
		t.Fatal(err)
	}
	...
	for _, f := range files {
		...
		targetPath := filepath.Join("cache", "hostname", f)

		if _, err := os.Stat(targetPath); err != nil {
			copyFile(t, originalPath, targetPath)
		}

		scaledPath := filepath.Join("cache", "hostname", getFilenameForSize(f, 16))
